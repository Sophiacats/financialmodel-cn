def _render_download_options(self, dcf_result: Dict, analyst_name: str, report_date: str):
        ...
        with col1:
            ...
            excel_data = self._create_complete_dcf_excel(dcf_result)
        ...
        with col2:
            ...
            # PowerPoint演示HTML版本
            ppt_html = self._generate_ppt_presentation(dcf_result, analyst_name, report_date)
            
            # 使用JavaScript打开PowerPoint
            ppt_js = f"""
            <script>
            function openPPTReport() {{
                var pptContent = `{ppt_html.replace('`', '\\`').replace('${', '\\${').replace('"', '\\"')}`;
    ...
    def _create_complete_dcf_excel(self, dcf_result: Dict) -> bytes:
        """创建完整的Excel DCF模型 - 恢复原版功能"""
        output = io.BytesIO()
            ...
            with pd.ExcelWriter(output, engine='openpyxl') as writer:
                ...
                for i in range(st.session_state.dcf_data['forecast_years']):
                    if i < len(st.session_state.dcf_data['revenue_growth_rates']):
                        ...
                        growth_rate = st.session_state.dcf_data['revenue_growth_rates'][-1] if st.session_state.dcf_data['revenue_growth_rates'] else 5.0
                    growth_data.append({
                        '年份': f'第{i+1}年',
                        '收入增长率(%)': growth_rate,
                ...
                if dcf_result:
                    ...
                    for i, year in enumerate(years):
                        if i < len(st.session_state.dcf_data['revenue_growth_rates']):
                            ...
                            growth = st.session_state.dcf_data['revenue_growth_rates'][-1] / 100 if st.session_state.dcf_data['revenue_growth_rates'] else 0.05
                        
                        revenue = revenue * (1 + growth)
                        fcf = revenue * st.session_state.dcf_data['fcf_margin'] / 100
                        discount_factor = 1 / ((1 + st.session_state.dcf_data['wacc']/100) ** year)
                    ...
                    # 4. 终值计算
                    terminal_fcf = detailed_forecast[-1]['自由现金流'] * (1 + st.session_state.dcf_data['terminal_growth']/100)
                    terminal_value = terminal_fcf / (st.session_state.dcf_data['wacc']/100 - st.session_state.dcf_data['terminal_growth']/100)
                    terminal_pv = terminal_value / ((1 + st.session_state.dcf_data['wacc']/100) ** st.session_state.dcf_data['forecast_years'])
    ...
    def _generate_ppt_presentation(self, dcf_result: Dict, analyst_name: str, report_date: str) -> str:
        """生成PowerPoint演示HTML - 恢复完整功能"""
        return f"""
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    ...
    def _render_dcf_report(self):
        ...
        if st.button("🔄 生成报告", type="primary"):
            with st.spinner("正在生成报告..."):
                ...
                # 生成完整的HTML报告
                report_html = self._generate_complete_dcf_report(
                    dcf_result, analyst_name, str(report_date), report_title
                )
                ...
                if st.components:
                    ...
                    st.markdown("**报告预览：**")
                    st.info("报告已生成，请使用完整的Streamlit环境查看HTML预览")
                
                # 提供下载选项
                self._render_download_options(dcf_result, analyst_name, str(report_date))
    
    def _generate_complete_dcf_report(self, dcf_result: Dict, analyst_name: str, 
                                    report_date: str, report_title: str) -> str:
        """生成完整的DCF HTML报告 - 恢复原版功能"""
        return f"""
<!DOCTYPE html>
<html lang="zh-CN">
<head>
...
<body>
    ...
    <div class="section">
        <h2>📋 执行摘要</h2>
        <p>基于贴现现金流(DCF)分析，{st.session_state.dcf_data['company_name']}的内在价值为<strong>{self.currency_symbol}{dcf_result['share_price']:.2f}每股</strong>。</p>
        ...
        <div class="metrics-grid">
            ...
            <div class="metric-card">
                <div class="metric-value">{self.currency_symbol}{dcf_result['share_price']:.2f}</div>
                <div class="metric-label">每股内在价值</div>
            </div>
            <div class="metric-card">
                <div class="metric-value">{(dcf_result['pv_terminal'] / dcf_result['enterprise_value'] * 100):.1f}%</div>
    ...
    <div class="section">
        ...
        <div class="assumptions">
            ...
            <ul>
                ...
                <li><strong>基期收入:</strong> {st.session_state.dcf_data['base_revenue']:.1f}百万{self.currency_symbol}</li>
            </ul>
        </div>
    </div>
    ...
    <div class="section">
        ...
        <table>
            <thead>
                <tr>
                    <th>年份</th>
                    <th>自由现金流(百万{self.currency_symbol})</th>
                    <th>贴现因子</th>
                    <th>现值(百万{self.currency_symbol})</th>
                </tr>
            </thead>
            <tbody>"""
        ...
        <table>
            <tbody>
                <tr><td>预测期现金流现值</td><td>{dcf_result['total_pv_fcf']:.1f}百万{self.currency_symbol}</td></tr>
                <tr><td>终值</td><td>{dcf_result['terminal_value']:.1f}百万{self.currency_symbol}</td></tr>
                <tr><td>终值现值</td><td>{dcf_result['pv_terminal']:.1f}百万{self.currency_symbol}</td></tr>
                <tr style="background-color: #e0f2fe;"><td><strong>企业价值</strong></td><td><strong>{dcf_result['enterprise_value']:.1f}百万{self.currency_symbol}</strong></td></tr>
                <tr><td>加: 现金及等价物</td><td>{st.session_state.dcf_data['cash']:.1f}百万{self.currency_symbol}</td></tr>
                <tr><td>减: 总债务</td><td>{st.session_state.dcf_data['debt']:.1f}百万{self.currency_symbol}</td></tr>
                <tr style="background-color: #e8f5e8;"><td><strong>股权价值</strong></td><td><strong>{dcf_result['equity_value']:.1f}百万{self.currency_symbol}</strong></td></tr>
                <tr><td>流通股数</td><td>{st.session_state.dcf_data['shares_outstanding']:.1f}百万股</td></tr>
                <tr style="background-color: #fff3cd;"><td><strong>每股内在价值</strong></td><td><strong>{self.currency_symbol}{dcf_result['share_price']:.2f}</strong></td></tr>
            </tbody>
        </table>
    </div>
